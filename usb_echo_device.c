...
* \version 1.0
*
* \brief    Implements the data loopback and source/sink logic for the FX2G3 USB Echo
*           Device application.
*
*******************************************************************************
* \copyright
...
#ifdef EXCLUSIVE_CFU_SPI
#include "spi.h"
#endif /* EXCLUSIVE_CFU_SPI */
 ...
 * \brief Initialize egress/write buffer to a value.
 * \param pWriteBuffer
 * \param len
 * \retval None
 */
 ...
 * \brief Initialize ingress/read buffer to a value.
 * \param pWriteBuffer
 * \param len
 * \retval None
 */
...
/**
 * \name Cy_InitIfxQueue
 * \brief Initialize queue which is part of echodevice context.
 ...
 * \brief Based on speed, need to update different pointer including
 *              data pointer.
 * \param pApp
 * \param devSpeed
 * \retval None
 ...
 * \brief Initialize EchoDevice for source/sink.
 * \param pAppCtxt
 * \retval None
 */
void  *
 ...
 * \brief It takes care of all preparation to start data transfer.
 * \param pAppCtxt
 * \retval None
 */
void
 ...
 * \brief It takes care of WRITE-COMPLETE message.
 * \param pAppCtxt
 * \param endpAddr
 * \retval None
 */
 ...
 * \brief It takes care of READ-COMPLETE message.
 * \param pAppCtxt
 * \param endpAddr
 * \retval None
 */
 ...
 * \brief It takes care of IN-ZLP message.
 * \param cy_stc_usb_app_ctxt_t
 * \param endpAddr
 * \retval None
 */
void
 ...
 * \brief It takes care of OUT-ZLP message.
 * \param pAppCtxt
 * \param endpAddr
 * \retval None
 */
...
#ifdef FX_CFU
    CFU_LOG_MESSAGE("CFU FEATURE: ENABLED");
    /* Start SPI, for SPI-based CFU */
    Cy_CFU_SPI_Start(pAppCtxt);
#else
    Cy_Debug_AddToLog(3, "[CFU]: CFU FEATURE: DISABLED\r\n");
#endif /* FX_CFU */
...
Cy_USB_EchoDeviceHandleCtrlSetup (void *pApp, cy_stc_usbd_app_msg_t *pMsg)
{
    ...
    switch (reqType) {
        ...
        case CY_USB_CTRL_REQ_STD:
            ...
            if((bRequest == CY_FX_USB_SC_GET_DESCRIPTOR) && (CY_FX_GET_MSB(wValue) == GET_REPORT_DESC))
            {
                retStatus = Cy_USB_USBD_SendEndp0Data(pAppCtxt->pUsbdCtxt, (uint8_t *)&cyFxHidReportDesc,0x81);
            ...
            if((bRequest == CY_FX_USB_SC_GET_DESCRIPTOR) && (CY_FX_GET_MSB(wValue) == GET_HID_DESCR))
            {
                retStatus = Cy_USB_USBD_SendEndp0Data(pAppCtxt->pUsbdCtxt, (uint8_t *)&cyFxHidInterfaceDescr, 0x09);
                if (retStatus == CY_USBD_STATUS_SUCCESS)
                {
                    isReqHandled = true;
                }
        ...
        case CY_USB_CTRL_REQ_CLASS:
            ...
            {
                DBG_APP_INFO("SET_IDLE\r\n");
                Cy_USBD_SendACkSetupDataStatusStage(pAppCtxt->pUsbdCtxt);
                isReqHandled = true;
            }
            if ((wIndex & 0xff) == HID_IF_NUM) /* HID Class Interface */
            {
                /* Handle this from the task handler */
                isReqHandled  = Cy_USB_HandleHidRequests(pAppCtxt->pUsbdCtxt,pAppCtxt,bRequest,wValue,wIndex,wLength);
            }
            break;
#endif /* FX_CFU */
